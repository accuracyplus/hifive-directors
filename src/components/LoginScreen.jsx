...
const keyBase = {
  height:'62px', borderRadius:'18px',
  background:'rgba(255,255,255,0.45)',
  backdropFilter:'blur(16px) saturate(180%)',
  WebkitBackdropFilter:'blur(16px) saturate(180%)',
  border:'1px solid rgba(255,255,255,0.6)',
  fontSize:'24px', fontWeight:'500', color:NAVY, cursor:'pointer',
...
export default function LoginScreen({ selUser, onSelectUser, pin, onKey, onBack, error, busy }) {
  ...
  return (
    <div style={{ flex:1, minHeight:0, display:'flex', flexDirection:'column' }}>
      ...
      {/* Body */}
      <div style={{ flex:1, minHeight:0, overflowY:'auto', padding:'30px 28px', display:'flex', flexDirection:'column', opacity: busy ? 0.7 : 1 }}>
        <div style={{ fontSize:'13px', fontWeight:'600', color:'#4b5563', marginBottom:'8px' }}>Select user</div>
        <select value={selUser} onChange={e=>onSelectUser(e.target.value)} style={{ ...INPUT_STYLE, marginBottom:'32px' }}>
        ...
        {/* Error message */}
        {error && (
          <div style={{ textAlign:'center', fontSize:'12px', color:'#c0392b', fontWeight:'600', marginBottom:'12px' }}>
            {error}
          </div>
        )}

        {/* Numpad */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'13px', maxWidth:'268px', margin:'0 auto', width:'100%' }}>
          {keys.map((k,i)=>{
            if (k===null) return <div key={i}/>
            if (k==='⌫') return <button key={i} onClick={onBack} style={{...keyBase,fontSize:'20px'}}>⌫</button>
            return <button key={i} onClick={()=>onKey(k)} style={keyBase} disabled={busy}>{k}</button>
          })}
        </div>

        <div style={{ textAlign:'center', marginTop:'26px', fontSize:'12px', color:'#9a958a' }}>
          {busy ? 'Verifying…' : 'Demo — any 4 digits unlock when offline'}
        </div>
      </div>
    </div>
  )
